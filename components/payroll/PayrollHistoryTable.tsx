...
import { BiFile, BiCheckCircle, BiHistory } from "react-icons/bi";
    ...
    <div className="relative overflow-x-auto bg-white dark:bg-gray-800 rounded-lg shadow-sm">
      <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody className="divide-y dark:divide-gray-700">
          {payrollRuns?.map((run: { id: string | number; month: number; year: number; departmentId: number; departmentName?: string; useUkProgressiveTax: boolean; taxRate: number; currency: string; status: string; isFinalized: boolean }) => (
            <tr key={run.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
      ...
      {payrollRuns?.length === 0 && !isLoading && (
        <EmptyState
          ...
          description="Your payroll cycle history is currently empty. Start your first payroll run to see historical data here."
        />
      )}
    </div>
  );
